...
export const calculateGasOf = async (txConfig, provider) => {
  try {
    const ethAdapter = new ethers.providers.Web3Provider(provider);
...
export const calculateGasPrice = async (currentChain, provider) => {
  ...
  const ethAdapter = new ethers.providers.Web3Provider(provider);
  return await ethAdapter.getGasPrice();
};
...
export const EstimateSafeCreationGas = async ({
  ...
  await estimateGas(
    ...
    gasEstimation.gasCostFormatted = res.gxasCostFormatted;
...
const estimateGasForTransactionExecution = async ({
  ...
  const safeInstance = new ethers.Contract(safeAddress, Safe.abi, signer);
  ...
  const estimationData = safeInstance
...
export const estimateGasForTransactionApproval = async ({
  ...
  const approveTransactionTxData = safeInstance.approveHash(txHash).encodeABI();
...
const createERC20TokenContract = (tokenAddress, provider) => {
  const ethAdapter = new ethers.providers.Web3Provider(provider);
  const signer = ethAdapter.getSigner();
  return new ethers.Contract(tokenAddress, ERC20Contract.abi, signer);
};

const createERC721TokenContract = (tokenAddress, provider) => {
  const ethAdapter = new ethers.providers.Web3Provider(provider);
  const signer = ethAdapter.getSigner();
  return new ethers.Contract(tokenAddress, ERC721Contract.abi, signer);
};
...
export const generateERC721TransferTxData = async (tx, safeAddress, currentChainId, provider) => {
  ...
  return NFTTokenInstance[methodToCall](...transferParams).encodeABI();
};
...
export const getUserNonce = async (userAddress, provider) => {
  const ethAdapter = new ethers.providers.Web3Provider(provider);
  try {
    return await ethAdapter.getTransactionCount(userAddress, "pending");
  } catch (error) {
